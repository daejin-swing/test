...
# ported from openpilot's system/sensord/sensors/lsm6ds3_accel.py -- register
# map and scaling are unchanged; get_event() returns (x, y, z, ts) in m/s^2
# instead of a capnp message (see i2c_sensor.py).
...
class LSM6DS3_Accel(Sensor):
  ...
  def get_event(self, ts: float | None = None) -> tuple[float, float, float, float]:
    ...
    # matches upstream's axis remap (v = [y, -x, z])
    return y, -x, z, ts
